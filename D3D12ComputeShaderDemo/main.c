...
static void CreateConstantBuffer(const void* inputData, size_t dataSize)
{
    D3D12_HEAP_PROPERTIES heapProperties = { D3D12_HEAP_TYPE_DEFAULT, D3D12_CPU_PAGE_PROPERTY_UNKNOWN,
    D3D12_MEMORY_POOL_UNKNOWN, 1, 1 };
    D3D12_HEAP_PROPERTIES heapUploadProperties = { D3D12_HEAP_TYPE_UPLOAD, D3D12_CPU_PAGE_PROPERTY_UNKNOWN,
    ...
    HRESULT hr = s_device->lpVtbl->CreateCommittedResource(s_device, &heapProperties, D3D12_HEAP_FLAG_NONE, &resourceDesc,
        D3D12_RESOURCE_STATE_COPY_DEST, NULL, &IID_ID3D12Resource, (void**)&s_constantBuffer);
    if (FAILED(hr))
        return;
    ...
    // Describe the data we want to copy into the constant buffer.
    D3D12_SUBRESOURCE_DATA computeCBData = { 0 };
    computeCBData.pData = inputData;
    computeCBData.RowPitch = dataSize;
    computeCBData.SlicePitch = computeCBData.RowPitch;
    UpdateSubresources(s_device, s_computeCommandList, s_constantBuffer, s_constantUploadBuffer, 0, 0, 1, &computeCBData);

    // Insert a barrier to sync the copy operation, 
    // and transit the SRV buffer to non pixel shader resource state.
    D3D12_RESOURCE_BARRIER barrier = { D3D12_RESOURCE_BARRIER_TYPE_TRANSITION, D3D12_RESOURCE_BARRIER_FLAG_NONE,
        .Transition = { s_constantBuffer, D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES,
